...
It is a polymorphic subtype of Entity (declaration: MixedOrderEntity u     Entity v) and a power instance of Class of Mixed-Order Entity (declaration:    ), 
which means that its subtypes may classify entities belonging to any universe level, whereas an Ordered Entity and its subtypes are anchored to a fixed level. 
A Mixed-Order Entity has no direct instances: it represents an aspect - a transversal characteristic  - that an ordered class acquires through the aspect relation (   ) without giving up its level anchoring. It is the SysFEAT counterpart of MLT*s orderless type, with two constitutive differences: transversality is carried by the universe polymorphism of the type system (rather than by a FOL axiomatization of integer levels), and cross-level classification goes through the controlled-reflexivity predicates (  ,   ), which makes it compatible with proof-kernel verification.
Example:
  1) Aspects of Entitys, such as Lexical Scope.
  2) Building Blocks.

Documentation : https://framework.sysfeat.com/pages/6ef572f868f1366f.htm
